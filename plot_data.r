...
total_confirmed = c(1000000, 2000000)
...
colXMax <- length(data$date)
plot(1, type="n", main="COVID-19 Mortality Rate %", axes=FALSE, xlab="", ylab="", xlim=c(0, colXMax), ylim=c(2, colYMax), las=2)
axis(1, at=1:colXMax, las=2, lab=data$date)
...
par(mar=c(6.1,5.1,4.1,2.1))
colYMax <- max(data$daily_growth_rate, na.rm = TRUE)
colXMax <- length(data$date)
plot(1, type="n", main="COVID-19 Global Daily Growth Rate %", axes=FALSE, xlab="", ylab="", xlim=c(0, colXMax), ylim=c(0, colYMax), las=2)
axis(1, at=1:colXMax, las=2, lab=data$date)
axis(2, at=seq(0, max(data$daily_growth_rate), by=0.1), las=1)
legend(1, colYMax, legend=c("Growth Rate %"),
       col=c("red"), lty=1:2, cex=0.8)
lines(data$daily_growth_rate~data$date, col='red', lwd=2)
dev.off()
